...
- Python执行SQL脚本
	...
	1. 游标对象的executescript()方法执行SQL脚本文件：
...
```
	...
	2. 直接使用数据库连接对象进行脚本执行
		...
		2.1 connection.execute(sql,[parameters])：执行一条SQL语句
		
		2.2. connection.executemany(sql, [parameters])：根据序列重复执行SQL语句
		
		2.3. connection.executescript(sql)：执行SQL语句
